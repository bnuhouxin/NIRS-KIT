function NR_nfri_mni_plot(CM, InputData, SimpleFlag)
...
MatFile = ...
    [fileparts(mfilename('fullpath')) filesep 'Brain2.mat'];
load (MatFile); % xall, yall, zall ... <1x68657>
ff=figure('Menubar', 'figure', 'Toolbar', 'figure') ;
DrawBrain([xall' yall' zall']);
...
Ans = 'No';%questdlg('Apply automatic scale? (Ignore the max and min value in the sheet?)', 'Automatic scale?');
DummyScale = 100;
...
% ff=figure('Menubar', 'figure', 'Toolbar', 'figure') ;
hToolbar = findall(ff,'Type','uitoolbar');
defaultSetDir = which('NIRS_REST');
index_dir=fileparts(defaultSetDir);
defaultSetDir=fullfile(index_dir,'file');
img = imread(fullfile(defaultSetDir,'Label.jpg'));
grid_control = uitoggletool(hToolbar,'CData',img,'TooltipString','Your toggle tool','HandleVisibility','off');
% grid_control = uitoggletool(hToolbar,'TooltipString','Your toggle tool','HandleVisibility','off');
set(grid_control, 'OnCallback',{@togglebuttonon});
set(grid_control, 'OffCallback',{@togglebuttondown});

%%

function togglebuttonon(hobj, event)
    ToggleLabel([], [])
function togglebuttondown(hobj, event)
    ToggleLabel([], [])
...
fineness=4;
lightpower=5;
scale = (40+lightpower)*fineness;
% colors = flipud(bone(scale));
%% wiz
base = [252,212,192]/255;
colors = [1:-1/scale:1e-7]'*base;
colors = colors(end:-1:1,:);
% colors = colors*1.5; colors(colors>1)=1;
%% end of wiz

center = mean(H, 1);
% dist = round(DistBtw(center, H)) - 48;
%% wiz
H2=H-ones(size(H(:,1)))*center;
H_scale=max(H)-min(H);
H2=H2.*( ones(size(H(:,1)))*(1./H_scale) )*min(H_scale);
dist = round(DistBtw([0,0,0], H2)*fineness) - (48-lightpower)*fineness;
%% end of wiz
