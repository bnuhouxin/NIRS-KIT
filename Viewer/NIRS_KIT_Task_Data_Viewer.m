function varargout = NIRS_KIT_Task_Data_Viewer(varargin)
% NIRS_KIT_TASK_DATA_VIEWER MATLAB code for NIRS_KIT_Task_Data_Viewer.fig
%      NIRS_KIT_TASK_DATA_VIEWER, by itself, creates a new NIRS_KIT_TASK_DATA_VIEWER or raises the existing
%      singleton*.
%
%      H = NIRS_KIT_TASK_DATA_VIEWER returns the handle to a new NIRS_KIT_TASK_DATA_VIEWER or the handle to
%      the existing singleton*.
%
%      NIRS_KIT_TASK_DATA_VIEWER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NIRS_KIT_TASK_DATA_VIEWER.M with the given input arguments.
%
%      NIRS_KIT_TASK_DATA_VIEWER('Property','Value',...) creates a new NIRS_KIT_TASK_DATA_VIEWER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before NIRS_KIT_Task_Data_Viewer_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to NIRS_KIT_Task_Data_Viewer_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help NIRS_KIT_Task_Data_Viewer

% Last Modified by GUIDE v2.5 20-Sep-2020 12:10:54
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @NIRS_KIT_Task_Data_Viewer_OpeningFcn, ...
    'gui_OutputFcn',  @NIRS_KIT_Task_Data_Viewer_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before NIRS_KIT_Task_Data_Viewer is made visible.
function NIRS_KIT_Task_Data_Viewer_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to NIRS_KIT_Task_Data_Viewer (see VARARGIN)

% Choose default command line output for NIRS_KIT_Task_Data_Viewer
...
% UIWAIT makes NIRS_KIT_Task_Data_Viewer wait for user response (see UIRESUME)
% uiwait(handles.viewer);


% --- Outputs from this function are returned to the command line.
function varargout = NIRS_KIT_Task_Data_Viewer_OutputFcn(hObject, eventdata, handles)
...
if get(handles.multipleMode,'value') == 1 || ~isempty(usd{1})
