function varargout = NIRS_BSS_open_file_and_config(varargin)
% NIRS_BSS_OPEN_FILE_AND_CONFIG M-file for NIRS_BSS_open_file_and_config.fig
%      NIRS_BSS_OPEN_FILE_AND_CONFIG, by itself, creates a new NIRS_BSS_OPEN_FILE_AND_CONFIG or raises the existing
%      singleton*.
%
%      H = NIRS_BSS_OPEN_FILE_AND_CONFIG returns the handle to a new NIRS_BSS_OPEN_FILE_AND_CONFIG or the handle to
%      the existing singleton*.
%
%      NIRS_BSS_OPEN_FILE_AND_CONFIG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NIRS_BSS_OPEN_FILE_AND_CONFIG.M with the given input arguments.
%
%      NIRS_BSS_OPEN_FILE_AND_CONFIG('Property','Value',...) creates a new NIRS_BSS_OPEN_FILE_AND_CONFIG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before NIRS_BSS_open_file_and_config_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to NIRS_BSS_open_file_and_config_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help NIRS_BSS_open_file_and_config

% Last Modified by GUIDE v2.5 02-Jul-2020 11:10:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @NIRS_BSS_open_file_and_config_OpeningFcn, ...
                   'gui_OutputFcn',  @NIRS_BSS_open_file_and_config_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before NIRS_BSS_open_file_and_config is made visible.
function NIRS_BSS_open_file_and_config_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to NIRS_BSS_open_file_and_config (see VARARGIN)

% Choose default command line output for NIRS_BSS_open_file_and_config
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes NIRS_BSS_open_file_and_config wait for user response (see UIRESUME)
% uiwait(handles.NIRS_BSS_open_file_and_config);


% --- Outputs from this function are returned to the command line.
function varargout = NIRS_BSS_open_file_and_config_OutputFcn(hObject, eventdata, handles) 
...
val = get(handles.spikelike,'Value');
...
if val
    ...
    set(handles.spikelike,'UserData','')
end
...
val = get(handles.external,'Value');
if val == 1
    datapath = get(handles.inpathData,'String');
    if isempty(datapath)
        errordlg('You need to input NIRS data first!','File Error')
...
% 
% if ischar(dataSource)
%     set(handles.external,'userdata',dataSource);
% end
...
set(handles.spikelike,'Value',1);
set(handles.homo,'Value',1);
set(handles.external,'Value',0);
%
