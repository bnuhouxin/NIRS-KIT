function NID_PLOT_Small_Timeserial(handles,data,order,ic_interest,numSort)
%% Plot separated time course in grid manner (small view)
% 
...
for i = firstNum:endNum
    ...
    %----- plot each feature -----%
    plot(ax_fig(ii),data(order(i),:),'k');
    set(ax_fig(ii),'XGrid','on','FontSize',5);
    set(ax_fig(ii),'YTick',[],'YColor',[1,1,1]);
    set(ax_fig(ii),'UserData',order(i));
    % 
    if i< (endNum-4)
        set(ax_fig(ii),'XColor',[0,0,0],'XGrid','on','FontWeight','light','FontSize',5);
        set(ax_fig(ii),'YTick',[],'YColor',[1,1,1]);
    end
    set(ax_fig(ii),'XLim',[0,length(data(i,:))])
    % 
        ...
        for j = 1:length(numSort)
            ...
            if label_tag
                switch temp
                    ...
                    case 4          % timeTemplate
                        set(handles.hinck,'BackgroundColor',[0,1,1]);
                    case 5          % spatialTemplate
...
%% ??????????????????????????????????????detail??
    ...
    width_box = 0.014;
    height_box = 0.021;
    start_detail = [0.21-width_box 0.951];
    move_y = 0.004;
    ...
    for i = firstNum:endNum
        ...
        startx = startx + width_box + stepx + 0.01;
        if mod(i,5) == 0
            starty = starty - height_box - height_ax - move_y;
            startx = start_detail(1);
        end
...
%% ?????????????? 
function label_Callback(hObject, eventdata,hinck,handles)
%
