...
%is diy_infomation be displayed, Edit by ZY
is_show_every_iter = 0;
is_show_dire = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     w = ones(vectorSize, 1);
	...
	drawnow;
      ...
      %% DIY Information:print basis
      if is_show_every_iter
          subplot(2,1,1);
          scatter(X(1,:),X(2,:),'+');
%           daspect([1,1,1]);
          hold on;
          vectarrow([0 0]',w,'r');    
          xlabel('mixed Variable 1');
          ylabel('mixed Variable 2');
           %print nonguassianity
    %       current_nong(i) = mean2((X' * w) .^ 4 );
          directionchange = norm(w - wOld);
    %       title(['nonguassianity:' num2str(current_nong(i)) ',  DirectionChange:' num2str(directionchange)]);
          title(['DirectionChange:' num2str(directionchange)]);
          subplot(2,1,2); 
          test = w' * X;
          plot(test);

          title('Demixed source signal');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIY By Zhaoyang plot every Value of estimation
if is_show_dire
    n = 1000;
    step = 2 * pi / n;
    v_j = zeros(n,1);
    w = [1 0]';
    gau_test = normrnd(0,1,[size(X,2) 1]);

    for i = 1:n
        switch g
            case 'pow3'
                v_j(i,1) = (mean2((X' * w) .^ 4) - 3).^2;
            case 'skew'
%                 v_j(i,1) = (mean2((X' * w) .^ 3) - mean(gau_test.^3)).^2;
                  v_j(i,1) = (mean2((X' * w) .^ 3)).^2;
            case 'tanh'
                v_j(i,1) = (mean(log(cosh(X' * w))) - mean(log(cosh(gau_test)))).^2;
            case 'gauss'
                v_j(i,1) = (mean(- exp( - (X' * w).^2/2)) - mean(- exp( - gau_test.^2/2))).^2;
        end
        w = [cos(i*step) sin(i*step)]';
    end
    thet = zeros(size(B,2),1);
    for ic = 1:size(B,2)
        thet(ic) = atan(B(1,ic)/B(2,ic));
        if thet(ic) < 0
            if B(1,ic) > 0
                thet(ic) = thet(ic) + 2*pi;
            else
                thet(ic) = thet(ic) + pi;
            end
        end
    ...
    figure();
    plot(0:2*pi/n:2*pi*(n-1)/n,v_j);
    set(gca, 'xtick', 0:pi/4:2*pi);
    set(gca, 'xticklabel', {'0','pi/4','pi/2','3/4*pi','pi','5/4*pi','3pi/2','7/4*pi','2pi'});
    % hold on;
    % for ic = 1:size(B,2)
    %     plot(thet(ic),0,'r+');
    %     hold on;
    % end
    xlabel('theta: angle of w');
    ylabel(['non-Gaussianity estimated by ' g ' function']);
end
