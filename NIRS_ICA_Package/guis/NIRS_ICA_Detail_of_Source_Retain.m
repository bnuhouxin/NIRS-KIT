function varargout = NIRS_ICA_Detail_of_Source_Retain(varargin)
% NIRS_ICA_DETAIL_OF_SOURCE_RETAIN M-file for NIRS_ICA_Detail_of_Source_Retain.fig
%      NIRS_ICA_DETAIL_OF_SOURCE_RETAIN, by itself, creates a new NIRS_ICA_DETAIL_OF_SOURCE_RETAIN or raises the existing
%      singleton*.
%
%      H = NIRS_ICA_DETAIL_OF_SOURCE_RETAIN returns the handle to a new NIRS_ICA_DETAIL_OF_SOURCE_RETAIN or the handle to
%      the existing singleton*.
%
%      NIRS_ICA_DETAIL_OF_SOURCE_RETAIN('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NIRS_ICA_DETAIL_OF_SOURCE_RETAIN.M with the given input arguments.
%
%      NIRS_ICA_DETAIL_OF_SOURCE_RETAIN('Property','Value',...) creates a new NIRS_ICA_DETAIL_OF_SOURCE_RETAIN or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before NIRS_ICA_Detail_of_Source_Retain_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to NIRS_ICA_Detail_of_Source_Retain_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help NIRS_ICA_Detail_of_Source_Retain

% Last Modified by GUIDE v2.5 11-May-2021 16:49:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @NIRS_ICA_Detail_of_Source_Retain_OpeningFcn, ...
                   'gui_OutputFcn',  @NIRS_ICA_Detail_of_Source_Retain_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before NIRS_ICA_Detail_of_Source_Retain is made visible.
function NIRS_ICA_Detail_of_Source_Retain_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to NIRS_ICA_Detail_of_Source_Retain (see VARARGIN)

% Choose default command line output for NIRS_ICA_Detail_of_Source_Retain
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes NIRS_ICA_Detail_of_Source_Retain wait for user response (see UIRESUME)
% uiwait(handles.NIRS_ICA_Detail_of_Source_Retain);


% --- Outputs from this function are returned to the command line.
function varargout = NIRS_ICA_Detail_of_Source_Retain_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes when NIRS_ICA_Detail_of_Source_Retain is resized.
function NIRS_ICA_Denoiser_Detail_of_IC_Reserve_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to NIRS_ICA_Detail_of_Source_Retain (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%     icdata.IC_weigthInfo(:,curicnum) = icdata.IC_weigthInfo(:,curicnum) * -1;
    ...
    % detail
    detailTag = findobj('Tag','NIRS_ICA_Denoiser_Detail_of_IC_Reserve');
    delete(detailTag)
...
%
% NIRS_ICA_Detail_of_Source_Retain                                  % Detail
% NID_Detail = findobj('Tag','NIRS_ICA_Detail_of_Source_Retain');
...
% %
% handles_all = get(handles.NIRS_ICA_Detail_of_Source_Retain,'Userdata');
% NIDHandles = handles_all.NIDHandles;
% 
% NID_Detial_IC_drawTimeseries_Reserve(handles,NIDHandles)
% NID_Detail_IC_drawFreq_Reserve(handles,NIDHandles)
...
% --- Executes when panel_ic_data is resized.
function panel_ic_data_SizeChangedFcn(hObject, eventdata, handles)
% hObject    handle to panel_ic_data (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
