function NID_Detail_NirsData_Set(curicNum,handles,NIDhandles)
%% NIRS denoise detail information
%% load data
curIC = curicNum;
...
%     probe2d = data.raw.probe2d;
    if ~isfield(data.raw,'probe2d') || isempty(data.raw.probe2d)
        LeftbottomPositon=[];
        for i = 1:Row
            for j = 1:10
...
if isempty(legendobj)
    legendbutton = uicontrol(handles.panel_nirs_data,'Style','togglebutton',...
           ...
           'TooltipString','Legend Switching',...
           'Units','normalized',...
...
if 1 == get(handles.nirs_data_multiplemode,'value')
    ...
    data.color = newcolor;
    set(handles.uipanel_bl,'UserData',data);
    %
