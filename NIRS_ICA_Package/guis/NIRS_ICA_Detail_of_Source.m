function varargout = NIRS_ICA_Detail_of_Source(varargin)
% NIRS_ICA_DETAIL_OF_SOURCE M-file for NIRS_ICA_Detail_of_Source.fig
%      NIRS_ICA_DETAIL_OF_SOURCE, by itself, creates a new NIRS_ICA_DETAIL_OF_SOURCE or raises the existing
%      singleton*.
%
%      H = NIRS_ICA_DETAIL_OF_SOURCE returns the handle to a new NIRS_ICA_DETAIL_OF_SOURCE or the handle to
%      the existing singleton*.
%
%      NIRS_ICA_DETAIL_OF_SOURCE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NIRS_ICA_DETAIL_OF_SOURCE.M with the given input arguments.
%
%      NIRS_ICA_DETAIL_OF_SOURCE('Property','Value',...) creates a new NIRS_ICA_DETAIL_OF_SOURCE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before NIRS_ICA_Detail_of_Source_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to NIRS_ICA_Detail_of_Source_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help NIRS_ICA_Detail_of_Source

% Last Modified by GUIDE v2.5 01-May-2021 07:12:01

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @NIRS_ICA_Detail_of_Source_OpeningFcn, ...
                   'gui_OutputFcn',  @NIRS_ICA_Detail_of_Source_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before NIRS_ICA_Detail_of_Source is made visible.
function NIRS_ICA_Detail_of_Source_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to NIRS_ICA_Detail_of_Source (see VARARGIN)

% Choose default command line output for NIRS_ICA_Detail_of_Source
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes NIRS_ICA_Detail_of_Source wait for user response (see UIRESUME)
% uiwait(handles.NIRS_ICA_Detail_of_Source);


% --- Outputs from this function are returned to the command line.
function varargout = NIRS_ICA_Detail_of_Source_OutputFcn(hObject, eventdata, handles) 
...
%initialize
    %config raw_data
    set(handles.NIRS_ICA_Denoiser_Detail_of_IC,'userdata',dataSource);
...
if 1 == get(handles.show_bl_remove,'Value')
...
if strcmp(multiviewMode,'Yes')
    ...
    curic = get(handledata.ax_fig,'Userdata');
    NID_Detail_NirsData_Set(curic,handledata.DetailHandles,handledata.NIDHandles)
    NID_nirsdata_drawTimeseries(handles)
    NID_nirsdata_drawFreq(handles)
else
end
...
% --- Executes when NIRS_ICA_Detail_of_Source is resized.
function NIRS_ICA_Denoiser_Detail_of_IC_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to NIRS_ICA_Detail_of_Source (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
set(handles.bl_user,'Value',0)

handlesAll = get(handles.NIRS_ICA_Denoiser_Detail_of_IC,'Userdata');
...
if isempty(icremove)
    errordlg('ICs being Removed are empty!','Error')
    set(handles.bl_raw,'Value',1)
    set(handles.bl_user,'Value',0)
    return
end
...
if ismember(targetic,icremove)
    errordlg(['Target Source(NO.',num2str(targetic),') can not be remove...'],'Error');
    set(handles.bl_raw,'Value',1)
    set(handles.bl_user,'Value',0)
    return
...
if any(icremove>numIC) || any(icremove<1)
    errordlg('IC NO. out of range...','Error');
    set(handles.bl_raw,'Value',1)
    set(handles.bl_user,'Value',0)
    return
...
NID_Detail_Get_BaseLine_NirsData(targetic,NIDhandles,handles)
...
% set(handles.show_bl_remove,'Value',0)
% 
% handlesAll = get(handles.NIRS_ICA_Detail_of_Source,'Userdata');
% NIDhandles = handlesAll.NIDHandles;
% 
% icdata = get(handles.ic_data_timeserials,'Userdata');
% targetic = icdata.icnum;
...
% bl_user_val = get(handles.bl_user,'Value');
% 
% if bl_user_val
%     handlesAll = get(handles.NIRS_ICA_Detail_of_Source,'Userdata');
%     ax_fig = handlesAll.ax_fig;
...
if ismember(targetic,icremove)
    warndlg(['Target IC(NO.',num2str(targetic),') can not be remove...'],'Warning');
    return
%     icremove(find(icremove==targetic)) = '';
end
if any(icremove>numIC) || any(icremove<1)
...
legend(handles.nirs_data_timeserial,'toggle')
...
handles_all = get(handles.NIRS_ICA_Denoiser_Detail_of_IC,'Userdata');
...
%
NID_Detail_NirsData_Set_icflip(icNum_next,handles,NIDHandles)
NID_Detail_IC_Set_icflip(icNum_next,handles,NIDHandles,order)
%
NID_nirsdata_drawTimeseries(handles)
NID_nirsdata_drawFreq(handles)
NID_Detial_IC_drawTimeseries(handles)
NID_Detail_IC_drawFreq(handles)
NID_Detial_IC_drawSpacialmap(handles)
NID_Detail_IC_Spacialmap_colorBar(handles)
...
% --- Executes on key press with focus on edit28 and none of its controls.
...
%     icdata.IC_weigthInfo(:,curicnum) = icdata.IC_weigthInfo(:,curicnum) * -1;
    ...
    % 
    NID_fileOpen = findobj('Tag','NIRS_ICA_Denoiser');
    ...
    % detail
    detailTag = findobj('Tag','NIRS_ICA_Denoiser_Detail_of_IC');
    delete(detailTag)
